...
export function useSnippetEditor(feature: string, file: string, nodeId: string, contentHash: string, onSaved: (tree: PageTree) => void) {
   ...
   * preview has been shown and explicitly confirmed. The hash sent is the one
   * this snippet was loaded against; before that load resolves it is the
   * tree's own hash (the same file, as loaded) — never an empty string, which
   * the server refuses outright (#590: 400 HASH_REQUIRED). */
