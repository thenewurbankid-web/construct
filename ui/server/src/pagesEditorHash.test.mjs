// #590 -- through the real route table: every Pages editor write that lands on disk REQUIRES the
// contentHash the client got when it loaded the page (the same contract `componentSave`,
// componentsApi.mjs, already answers for the Components screen). No hash is 400 HASH_REQUIRED; a hash
// for an older copy of the file is 409 CHANGED_ON_DISK; the file on disk is untouched in both cases;
// the matching hash saves exactly as before.
...
// Every disk-writing Pages editor route, with a body that would otherwise succeed once the hash is
// right. Each is exercised with the hash left out (undefined/empty/null) and with a stale one; the
// file on disk must not move in either case.
const WRITES = [
