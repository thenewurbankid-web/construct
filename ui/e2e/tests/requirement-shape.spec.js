...
test.describe.serial('Requirement: the screen-shape offer (q-shape) is drawn and answered (#651)', () => {
  ...
  test('choosing scaffold shows fewer files; choosing list shows the 7-step plan, the typed file names and who decided (person)', async ({ page }) => {
    await gotoCockpit(page, '/requirement');
    await readSentence(page, PRODUCTS);
    ...
    expect(list.body.plan.steps.map((s) => s.flow)).toEqual(['create.feature', ...Array(6).fill('create.unit')]);
    expect(list.body.plan.steps.slice(1).every((s) => s.args.shape === 'list' && s.args.entity === 'Product')).toBe(true);
  ...
  test('Approve with the shape chosen starts a process whose steps carry --shape list, and nothing reaches the project', async ({ page, request }) => {
    ...
    expect(detail.process.steps.map((s) => s.flow)).toEqual(['create.feature', ...Array(6).fill('create.unit')]);
    ...
    for (const c of commands.slice(1)) expect(c).toContain('--shape list --entity Product --fields id:string,name:string,price:number');
