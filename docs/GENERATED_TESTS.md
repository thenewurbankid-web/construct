...
No second source of truth: the spec file *is* the document. `src/engine/testSteps.mjs` parses the test body with `src/ast` into steps and re-renders it through `src/engine/testSpecRender.mjs`, the same renderer the generator uses.
