...
| `chooser` | `{ id, question }`. Card questions: `requirement.card.noun` / `.verb`. Placement: `requirement.placement.ambiguity` / `.check` / `.server-check` / `.shape`. The wiring of a shaped plan (#654, `choicesFromWiring`): `requirement.plan.route` / `.dependency` / `.source` (#621, where a shaped screen reads its data from) / `.env` (#632, `q-env` and `q-env-<name>`: add a variable to `.env.example` or not) / `.verify` (#632, `q-verify`: type-check, type-check and build, or none) / `.steps` (#659, `q-steps`: a wizard's step count) / `.states` (#622, `q-states` and `q-states-<name>`: default views, a component of your own, or a skipped view for a screen's loading, empty and error states) / `.access` (#629, `q-access`: public, signed-in or a role list for a route) / `.state` (#630, `q-state`: a client-state store of a shape, or none) / `.handler` (#625, `q-handler`: add the Next.js route handler that serves the endpoint a screen calls, or not). A chain: the chooser's own id. |
