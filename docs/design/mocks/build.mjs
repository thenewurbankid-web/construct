...
const topbar = (mode, procs = '2 running', extra = '') => `
<header class="topbar">
...
const drawerBar = (on, counts = ['3', '', '2']) => tabs([['Diagnostics', [counts[0], 'danger']], ['Logs', null], ['Processes', [counts[2], 'acc']]], on);
...
const shell = ({ mode = 'Explore', left, mid, right, drawer, dim = false, overlay = '', drawerH = 132, procs, lw, rw }) => `
...
// 7 — states + narrow
const stateCard = (icon, h, p, extra = '') => `<div class="state-card"><div class="ico">${icon}</div><h5>${h}</h5><p>${p}</p>${extra}</div>`;
