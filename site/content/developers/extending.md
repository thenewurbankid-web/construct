...
1. **Declare it** in `DEFAULT_RULES` in `packages/core/config.mjs`: `'MY-001': { severity: 'error', name: 'What it enforces' }`. That makes it configurable in `architecture.yml` and gives it "did you mean" suggestions for typos.
2. **Check it** in the enforcer that owns the concern (`architecture-enforcer.mjs`, `soc-enforcer.mjs` or `readability-enforcer.mjs`). Read the configured severity from `config.rules[id]`, honour `off` and `exceptions:` (the architecture enforcer's `pushViolation` does both), and report with `makeViolation` from `packages/core/diagnostics.mjs`. Every violation needs a `why` and, where possible, `expected` and `suggestedFix`: the message is aimed at a person or model who has to fix it.
...
Generated files come from templates in `packages/core/generators.mjs`. Each layer has a template, a folder, a file-name convention (`layerFileBaseName`) and a constraint string (`LAYER_CONSTRAINTS`) that is shown verbatim to any model asked to fill the file. Generation order is `LAYER_ORDER`.
...
Providers live in the `PROVIDERS` map in `packages/core/llm.mjs`: a name mapped to `async (prompt, options) => string`. Add an entry and `--llm <name>` accepts it; nothing else changes, because every model call in Construct goes through `callLlm`. Throw `ConstructError` for failures. Replies written into files also pass through `packages/core/llm-fill.mjs`, which requires them to parse before they are written. Keep providers out of everything except this module so the rest of the tool stays repeatable.
...
Commands are dispatched in `packages/cli/construct.mjs` to functions exported from `packages/core/cli.mjs`. Add the function, add its usage to `packages/core/usage.mjs` (the [CLI reference](@developers/cli-reference/) is generated from it), and mention it in the README's command list.
