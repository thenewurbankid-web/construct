...
- **One entry point per block.** Each area exposes a single module (`src/ast/index.mjs`, `src/engine/pipeline.mjs`, `src/config.mjs`, ...) so it can be reused by the CLI, the UI backend and scripts.
