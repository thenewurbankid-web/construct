...
The **Tests** screen in the left rail lists every route through a feature's workflow, worked out from the flow itself, and whether each has a test. Generated tests are locked. To change one you clone it, and the clone is yours and nothing regenerates it. It keeps a record of the scenario it came from.
...
Pick a feature in the Browser pane. The table has one row per route through its workflow, with the branch it takes, whether a locked generated test exists (**Locked**) or not (**None**, with a **Generate** button), and its last result (**Not run** until something has run it). The Browser pane splits tests into **Generated** (locked) and **Yours**. A feature with routes but no tests yet shows one button, **Generate N tests**.

**Run all tests** above the table (or **Run this test** on a selected test) runs them against your own app at the address under **The app runs at**: start the app first, and only an address on this machine is accepted.
...
Select a generated test and press **Clone to edit**, or press **Edit** on one of its steps: a dialog ("This test is generated: clone it to edit") explains why it is locked and asks for a name. **Create clone and open** writes the copy into your area of the feature's tests; it is never overwritten and keeps a record of which scenario it came from, so a **Clone out of date** chip can tell you later that the flow changed and your copy drifted. **Just show me the code** shows the read-only source instead.
...
Select one of your tests and press **Edit steps**. It opens as a list of Given / And / When / Then / Check rows, each showing the selector it binds to. A form beside the list edits the selected row using pick-lists of the flow's real events and real states. Add a flow event, a flow state or a check (**+ Add flow event**, **+ Add flow state**, **+ Add check**); remove a step (it stays struck through, with **Put back**, until you save); reorder with **Up** and **Down**.
