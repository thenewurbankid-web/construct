...
function transitionsOf(value, what) {
  ...
  const s = strOf(value);
  if (s !== undefined) return [{ target: s }];
  if (value.type === 'ObjectExpression') {
    const p = propsOf(value, what);
    const tNode = p.get('target');
    let targets;
    if (tNode && tNode.type === 'ArrayExpression') targets = tNode.elements.map(strOf).filter((x) => x !== undefined);
    else if (tNode) {
    ...
    return targets.map((target) => ({ target, guard }));
  }
...
function extractMachine(call, exportName) {
  ...
  try {
    ...
    machine.states = raw.map(({ _node, ...rest }) => ({ ...rest, final: rest.type === 'final' }));
  } catch (e) {
...
/** Extract every XState machine defined in `source`. Always returns
...
export function extractMachines(source) {
  ...
  walkAst(ast, {
    enter(node, parent) {
      ...
      try {
        machines.push(extractMachine(node, exportName));
      } catch (e) {
