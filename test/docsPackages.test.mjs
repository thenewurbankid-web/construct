...
import { PACKAGES, findDocsRepoRoot, packageForRelativePath, docsPathFor } from '../packages/core/docsPackages.mjs';
import { makeTempDir } from '../test-utils/tmpdir.mjs';

const REPO_ROOT = path.resolve(import.meta.dirname, '..');

test('PACKAGES stays in sync with packages/docs-site/lib/apiDocs.mjs API_PACKAGES (id + dirs)', async () => {
  const { API_PACKAGES } = await import('../packages/docs-site/lib/apiDocs.mjs');
  ...
  for (const pkg of PACKAGES) {
    ...
    assert.deepEqual(pkg.dirs, real.dirs.map((d) => d.dir), `${pkg.id}: dirs match packages/docs-site/lib/apiDocs.mjs`);
  }
});

test('packageForRelativePath: longest-prefix match, no match for an unrelated path', () => {
  assert.equal(packageForRelativePath('packages/engine/pipeline.mjs').id, 'engine');
  assert.equal(packageForRelativePath('packages/core/cli.mjs').id, 'core');
...
test('docsPathFor: a real path in this repo resolves; an ordinary project never does', () => {
  ...
  assert.equal(docsPathFor(REPO_ROOT, path.join(REPO_ROOT, 'packages/engine/pipeline.mjs')), 'developers/api/engine/');
  const plain = makeTempDir('docs-packages-test-');
