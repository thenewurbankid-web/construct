...
#  - No interactive Claude Code session in this repo: start OG in a detached tmux session with Remote Control enabled.
...
#   install    copy this script to ~/.og-watchdog and add the cron entry (every 3 minutes); rerun after editing
...
start_og() {
  ...
  name="og-$(hostname -s)"
  if [ "$dry" = "--dry-run" ]; then
    echo "would run in tmux '$TMUX_NAME' (cwd $REPO): $CLAUDE_BIN --agent og --permission-mode auto --remote-control=$name -n og \"\$(cat $run)\""
    return 0
  fi
  record_start
  ...
  tmux new-session -d -s "$TMUX_NAME" -c "$REPO" \
    "exec env -u GH_TOKEN -u GITHUB_TOKEN '$CLAUDE_BIN' --agent og --permission-mode auto --remote-control='$name' -n og \"\$(cat '$run')\"" 9>&- \
