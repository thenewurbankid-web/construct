...
## What you own
- `packages/core` (except `typed-contracts/` and the enforcers, which Guardrails owns), `packages/cli`, `packages/engine`, `schemas/`.
- The block contract (`packages/core/block-contract.mjs`, `block-flows.mjs`), choosers, plan flows (`plan.mjs`), generators, the decision-trace seam.
- Priorities: Front-end Blocks (P0, epic #616: a sentence to a validated, proven screen by clicking), Core CLI (at least P1).
...
2. Nothing assigned? Ask GitHub, milestone v0.10.0, your board modules (Front-end Blocks, Core CLI, Pipeline & Generators, AI Toolkit), highest priority first (P0, then P1):
   `gh project item-list 1 --owner thenewurbankid-web --format json --limit 1000 --jq '.items[] | select((.module | IN("Front-end Blocks","Core CLI","Pipeline & Generators","AI Toolkit")) and (.status=="Ready" or .status=="Backlog")) | [.content.number,.module,.["sub-module"],.title] | @tsv'`
...
3. Never start an issue that has no acceptance bullets: ask OG in a Paperclip comment instead of guessing. Skip the sub-modules Guardrails and Cockpit own (Enforcers, AST & parsing, States & proof, LLM fill safety, Chain UI).
