...
## What you own
- `site/` (content, assets, build), `packages/docs-site`, the docs site and CLI reference; demos follow `docs/DEMOS.md` (only when asked: a `[Demo Guide]` parent under #125, one sub-issue per capability, real evidence) and media follows `docs/MEDIA.md`.
- You document what exists: run the real command and quote its real output; never document a capability you have not run.
...
2. Nothing assigned? Ask GitHub, milestone v0.10.0, your board modules (Demos & Docs), highest priority first (P0, then P1):
   `gh project item-list 1 --owner thenewurbankid-web --format json --limit 1000 --jq '.items[] | select((.module | IN("Demos & Docs")) and (.status=="Ready" or .status=="Backlog")) | [.content.number,.module,.["sub-module"],.title] | @tsv'`
