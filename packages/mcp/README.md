...
| `summarize` | `construct summarize`: per feature its lines of code, public API and files per layer, plus the compact paragraph. With `backend: true`, `construct summarize --backend` for the project's Node.js / Express backend (`backend.dir` in `architecture.yml`, else the root): the route table, role and effect counts, environment variable names, import cycles and what was not detected, at most 80 routes | `{ "feature": "billing" }` or `{ "backend": true }` |
