 ...
 * @returns {Promise<object>} `{ ok, stage, complete, card, blocks, questions, offers, decisions, plan, files, proof, wiring, env, guards, stores, handlers, verify, warnings, notes, apply }`.
...
export async function placementPlace(ctx, { text, answers = [] }) {
  ...
  return {
    ...
    handlers: (planned.handlers ?? []).slice(0, 5).map((h) => ({ name: h.name, method: h.method, path: h.path, question: h.question, step: h.step })),
