...
| staleness | `collectControllerStateCalls(ast)`, `collectUncleanedSubscriptions(ast)`, `collectParamsReads(ast)` | the three stale-input shapes of story #577: a state/ref/effect/memo hook call in a controller (CONTROLLER-003), a `useEffect` that starts a listener/timer/subscription and returns no cleanup (HOOK-003), a read into `params`/`searchParams` in a route (ROUTE-003); each `{name|api, index}[]` sorted by offset |
