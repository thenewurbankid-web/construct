...
function workflowFile(ctx) {
  ...
  return lines(
    ...
    '  },', '});', '',
    `/** The ${singular} workflow unit: the machine's id, first step and states, registered under its name for tooling. The hook runs \`${names.machine}\`. */`,
    `export const ${names.workflow} = defineWorkflow<Record<string, never>>('${names.workflow}', () => ({ id: ${names.machine}.id, initial: '${first}', states: ${names.machine}.config.states ?? {} }));`,
  );
}
...
function hookFile(ctx) {
  ...
  return lines(
    ...
    `import { ${names.stepsOf}, ${names.toInput} } from '../domain/${names.Name}.domain';`, `import { ${names.describe} } from '../domain/${names.Name}Screen.domain';`, `import { ${names.submit} } from '../services/${names.Name}.service';`, `import { ${names.machine} } from '../workflows/${names.Name}.workflow';`,
    `import type { ${names.event}, ${names.state}, ${names.values} } from '../types';`, '',
    `const INITIAL = getInitialSnapshot(${names.machine});`, '',
    ...
    `  const send = (event: ${names.event}) => setSnapshot((current) => getNextSnapshot(${names.machine}, current, event));`,
    '  useEffect(() => {',
    ...
    `      setSnapshot((current) => getNextSnapshot(${names.machine}, current, result.status === 'submitted' ? { type: 'SUCCEEDED' } : { type: 'FAILED', message: result.message }));`,
