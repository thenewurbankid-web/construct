...
export const PLAN_FLOWS = Object.freeze({
  ...
  'guard.route': {
    ...
    args: {
      ...
      redirect: { type: 'string', flag: '--redirect', description: 'An address on this site the fallback links to, for example /sign-in (not a path of a file). Without it the fallback is a notice.' },
      route: { type: 'string', flag: '--route', description: 'The route the screen is served at, for example /products (not a path of a file). Defaults to the kebab-case of the name.' },
      dir: DIR_ARG,
    },
  },
  'create.store': {
  ...
  'create.handler': {
    ...
    args: {
      ...
      path: { type: 'string', required: true, flag: '--path', description: 'The address of the endpoint, starting with /api/ and lower-case segments, like /api/products (not a path of a file).' },
