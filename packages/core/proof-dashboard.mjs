...
export function dashboardProofText(ctx, request, relPath, kit) {
  ...
  const L = [
    ...
    `  expectShown('The heading', html, ${lit(`<h1>${ctx.heading}</h1>`)});`,
    '  const tiles: [string, string][] = [',
    ...tiles.map((t) => `    [${lit(t.label)}, ${lit(tileHtml(t))}],`),
    '  ];',
    "  tiles.forEach(([label, markup]) => expectShown('The tile \"' + label + '\"', html, markup));",
    ...
    "    expectShown('The panel \"' + title + '\"', html, opening);",
    "    lines.forEach((line) => expectShown('A line of the panel \"' + title + '\"', html, line));",
    '  });',
    '});',
    '',
